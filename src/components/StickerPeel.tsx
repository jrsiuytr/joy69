...
export function StickerPeel(props: StickerPeelProps) {
  ...
  const updateBones = useCallback(() => {
    if (!bonesRef.current.length || !meshRef.current || !bonesInitialPositionsRef.current.length)
      return;
  ...
  const buildTransitionConfig = useCallback((transitionValue: any) => {
    ...
    if (transitionValue?.type === 'spring') {
      ['stiffness', 'damping', 'mass', 'bounce', 'restDelta', 'restSpeed', 'duration'].forEach(
        (k) => {
  ...
  const handlePointerUp = useCallback(() => {
    ...
    if (onClick) onClick();
  }, [hoverPeel, animateCurlTo, onClick]);
  ...
  useEffect(() => {
    ...
    return () => cleanupScene();
  }, [setupScene, loadTexture, cleanupScene]);
